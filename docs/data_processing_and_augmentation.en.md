...
For `vul_line`, released line labels use `context_relative_1based` coordinates: `vulnerabilities[].line` and `line_end` are 1-based line numbers relative to the released `context` field. Original source-file coordinates are provided as `source_line` and `source_line_end` when recoverable from raw source metadata or full-context identity mapping, and `source_mapping_method` records the mapping basis. The first manuscript evaluates representative vulnerable start lines; range-aware evaluation using `line_end` is reserved for future versions.
