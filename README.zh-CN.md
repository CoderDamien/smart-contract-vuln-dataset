...
对于 `vul_line`，`vulnerabilities[].line` 和 `vulnerabilities[].line_end` 是相对于已发布 `context` 字段的 1-based 行号，不一定是原始源码文件行号。若可还原原始源码坐标，则通过 `source_line` 和 `source_line_end` 提供；否则 `source_mapping_status` 为 `unavailable`。
